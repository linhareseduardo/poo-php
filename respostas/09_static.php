...
class Configuracao {
    ...
    public static function set($chave, $valor) {
        // Usando self:: para acessar a propriedade estática.
        self::$config[$chave] = $valor;
    }

    public static function get($chave) {
        return self::$config[$chave] ?? null; // Retorna null se a chave não existir
    }
}

// Não é necessário criar um objeto: $config = new Configuracao();

// Chamando os métodos estáticos diretamente na classe.
Configuracao::set('site_url', 'http://meusite.com');
Configuracao::set('db_user', 'admin');

echo "URL do Site: " . Configuracao::get('site_url') . "\n";
echo "Usuário do BD: " . Configuracao::get('db_user') . "\n";
